...
class SSA_Shortcodes {
	...
	public function ssa_confirmation() {
		...
		// else continue to show ssa_booking - consider appointment abandoned
		if ( ! empty( $paypal_success ) || ! empty( $paypal_cancel ) ) {
			$atts = array_merge(array( 'view' => "confirm_payment", 'payment_provider' => 'paypal' ), $atts);
		}

		
		if( isset( $_GET['redirect_status'] ) && $_GET['stripe_payment'] === 0 ){
			// We don't care at this point whether the payment was authorized or not
			// just make sure to show the user confirm_payment view if this was a stripe payment redirect
	...
	public function ssa_booking( $atts, $is_embedded_page = false ) {
		...
		if( $stripe_payment ){
			...
			// we decided to not trust redirect status in the url,
			// and instead make the frontend wait for either confirmation or failure of payment
			// this process is the same with PayPal
			return $this->ssa_confirmation();
		}
